	...
	$(document).ready( function () {
		...
		var table = $('#myTable').DataTable({
			...
			columns: [ 
				...
				{ data: 'AlamatPenerima' },
			 	{ 
			 		data: 'SistemPembayaran',
			 		render: function(data, type, row) {
			 			if (data === '1') {return 'Pembayaran Di Tempat'}
			 			if (data === '2') {return 'COD'}
			 			if (data === '3') {return 'Pembayaran Nanti'}
			 		}
			 	},
			 	{ 
			 		data: 'Berat',
		...
		function validation() {
			if ($('#NoResi').val() == '') {
				...
				return false;
			}else if ($('#Sistem Pembayaran').val() == '') {
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: "Mohon Pilih Sistem Pembayaran",
				});
				return false;
			}else if ($('#Harga').val() == '') {
				Swal.fire({
		...
		$("#addPenerimaan").on("click", function() {
			...
			$('#KodeKota').trigger('change');
			$('#AlamatPenerima').val('');
			$('#SistemPembayaran').val('');
			$('#SistemPembayaran').trigger('change');
			$('#Berat').val('');
			$('#Harga').val('');
			$('#Status').val('');
			...
			if (validation() == true) {
				$.ajax({
					...
					data: "NoResi="+$('#NoResi').val()+
						  ...
						  "&SistemPembayaran="+$('#SistemPembayaran').val()+
						  "&Berat="+$('#Berat').val()+
						  "&Harga="+$('#Harga').val()+
						  "&Status="+$('#Status').val(),
					dataType: "json",
		...
		$("#myTable").on("click", 'td.updatePengiriman', function() {
			...
			$('#SistemPembayaran').val(data.SistemPembayaran);
			$('#SistemPembayaran').trigger('change');
