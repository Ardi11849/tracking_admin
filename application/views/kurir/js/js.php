...
<script type="text/javascript">
	$(document).ready( function (){
		...
		$("#addForm").on("click", function() {$('#addForm').prop('disabled', true);
			if (validation() == true) {
				$.ajax({
					type: "post",
		...
		$("#updateForm").on("click", function() {$('#updateForm').prop('disabled', true);
			if (validation() == true) {
				$.ajax({
					type: "post",
					url: "<?php echo base_url() ?>Kurir/put_kurir",
					data: "IdKurir="+$('#idKurir').val()+
						  "&Nama="+$('#nama').val()+
						  "&NoTelp="+$('#noTelp').val(),
					dataType: "json",
					success: function (hasil) {
		...
		$("#myTable").on("click", 'td.deleteKurir', function() {
			console.log(table.row(this).data());
			const data = table.row(this).data();
			$('td.deleteKurir').prop('disabled', true);
			Swal.fire({
				title: 'Are you sure?',
				...
				$('td.deleteKurir').prop('disabled', false);
