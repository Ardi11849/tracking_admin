...
<script type="text/javascript">
	$(document).ready( function () {
			...
			columns: [
				...
				{ 
					...
					render: function(data, type, row){
						if (data == 1) {return "Terkirim"}else{return "Dalam Proses"}
					}
				},
				{ data: 'Diterima' },
				{ 
		...
		$('#import').on('click', function() {
			...
			$.ajax({
				url: '<?php echo base_url();?>PengirimanKurir/upload',
				dataType: 'text',
				cache: false,
				contentType: false,
				processData: false,
		...
		$("#myTable").on("click", 'td.updatePenerima', function() {
			...
			$('#NoHpPengirim').val(data.NoTelpPengirim);
		...
		$("#updateForm").on("click", function() {
			...
			if (validation() == true) {
				$.ajax({
					...
					data: "IdPengirimanKurir="+$('#IdPengirimanKurir').val()+
						  "&NoResi="+$('#NoResi').val()+
						  "&IdKurir="+$('#IdKurir').val()+
						  "&Penerima="+$('#Penerima').val()+
						  "&Alamat="+$('#Alamat').val()+
						  "&NoHpPenerima="+$('#NoHpPenerima').val(),
					dataType: "json",
